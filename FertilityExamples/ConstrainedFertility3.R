...
## replace when WEI==0 with arbitrary number
E[WEI==0] <- 10
Y[WEI==0] <- 10
...
## compute observed MAB
MAB <- apply(MX, 2, 
             function(mx) sum(mx*(x+0.5)) / sum(mx))
MAB.tar <- MAB#+0.2#runif(n, -0.5, 0.5)
plot(t, MAB, ylim=range(MAB, MAB.tar))
points(t, MAB.tar, col=2, pch=2)
...
plot(t, TFR, col=1, pch=1,
     ylim=range(TFR, TFR.tar, TFR.S))
points(t, TFR.tar, col=2, pch=2)
points(t, TFR.S, col=3, pch=3)
legend("top", c("True", "Target", "Smooth"), col=1:3, pch=1:3)

## compute and plot estimated MAB
MAB.S <- apply(MU.S, 2, 
               function(mx) sum(mx*(x+0.5)) / sum(mx))
plot(t, MAB, col=1, pch=1,
     ylim=range(MAB, MAB.tar, MAB.S))
points(t, MAB.tar, col=2, pch=2)
points(t, MAB.S, col=3, pch=3)
legend("top", c("True", "Target", "Smooth"), col=1:3, pch=1:3)
...
whi <- floor(seq(1,n,length=4))
rany <- range(MX, MU.T, MU.S, MU.C, 0)
par(mfrow=c(2,2))
for(i in 1:4){
  plot(x, MX[,whi[i]], pch=16, 
