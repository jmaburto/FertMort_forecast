...
## compute summary measures with matrices on MX
ones.m <- matrix(1,m,1)
ones.n <- matrix(1,n,1)
# mx1 <- MX1[,1]
# 
...
# 
# xM <- matrix(x+0.5,m,1)
# mab1 <- t(mx1)%*%xM / t(ones.m)%*%mx1
# mab1
# MAB1[1]
# 
# VAB1[1]
# t(mx1)%*% (xM^2) / t(ones.m)%*%mx1 - (t(mx1)%*% xM / t(ones.m)%*%mx1)^2
...
MU.PS <- exp(ETA.PS)
TFR.PS <- colSums(MU.PS)
MAB.PS <- apply(MU.PS, 2, 
                function(mx) sum(mx*(x+0.5)) / sum(mx))
VAB.PS <- numeric(n)
for(i in 1:n){
  mx.i <- MU.PS[,i]
  mab.i <- MAB.PS[i]
  VAB.PS[i] <- sum((((x+0.5) - mab.i)^2)*mx.i)/sum(mx.i)
}
