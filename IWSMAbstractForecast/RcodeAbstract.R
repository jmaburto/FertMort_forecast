...
library(vars)
library(forecast)
load("/home/camarda_cg/WORK/FertMort/trunk/JAPfem.RData")
...
lege2 <- expression(paste("SQP + 2D P-splines, ", e^0, "=90.47"))
...
library(vars)
library(forecast)
load("/home/camarda_cg/WORK/FertMort/trunk/ESPfert.RData")
...
lege1 <- expression(paste("2D P-splines, ", TFR, "=2.21"))
lege2 <- expression(paste("SQP + 2D P-splines, ", TFR, "=1.45"))
legend("topleft", inset=0.05, c(lege1, lege2),
       lwd=c(3,4), lty=c(2,1), col=c(2,4),
       bg="grey95", cex=1.7)
dev.off()                    
